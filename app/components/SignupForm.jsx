import { User, Sparkles, Mail, Lock, ArrowRight, CheckCircle } from "lucide-react";

export default function SignupForm({ formData, setFormData, handleSubmit, loading, error, success }) {
    return (
        <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-4 animate-fade-in">
                <div className="relative group">
                    <User className="absolute left-3 top-3.5 text-slate-400 group-focus-within:text-indigo-400 transition-colors" size={18} />
                    <input
                        type="text"
                ...
                <div className="relative group">
                    <Sparkles className="absolute left-3 top-3.5 text-slate-400 group-focus-within:text-indigo-400 transition-colors" size={18} />
                    <input
                        type="text"
                        placeholder="Current Goal (e.g. Learn Physics)"
                        required
            ...
            >
                {loading ? <span className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin"></span> : <>Create Account {!success && <ArrowRight size={18} />}</>}
            </button>
        </form>
    );
}
